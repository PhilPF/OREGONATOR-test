...
#define verbose_OD_magn ( 0 || verbose || verbose_OD_info ) && !verbose_none
#define verbose_OD_time ( 0 || verbose || verbose_OD_info ) && !verbose_none
#define verbose_OD_pos  ( 0 || verbose || verbose_OD_info ) && !verbose_none
#define verbose_OD_ssc  ( 0 || verbose || verbose_OD_info ) && !verbose_none
#define verbose_OD_part ( 0 || verbose || verbose_OD_info ) && !verbose_none
...
int main(int argc, char *argv[]){
    ...
    char OD_num_name[] = "ORBIT DECOMP/num_OD.txt", OD_part_name[] = "ORBIT DECOMP/partition.bin", OD_orbit_name[] = "ORBIT DECOMP/orbit.bin", OD_A_name[] = "ORBIT DECOMP/A.bin", OD_A_dat_name[] = "ORBIT DECOMP/A.dat";
    ...
    int OD_partition_bool=0, OD_last_rejected=0, OD_AS=0, OD_RS=0, OD_RK_RS=0;
    double OD_stepsize=t_F/OD_H0_frac;
    ...
    while(t<t_F){
        ...
        if (OD_partition_bool){
            ...
            condition(&OD_max_magnitude, &OD_min_magnitude, tmp, NULL);
    ...
    printf("Num of OD:%d, with %d accelerations and %d decelerations (%d reached RK stepsize)\n", od, OD_AS, OD_RS, OD_RK_RS);
